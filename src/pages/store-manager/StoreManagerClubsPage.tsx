...
const StoreManagerClubsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStoreClubs = async () => {
      // Wait for store access loading to complete
      if (storeAccessLoading) return;

      // Only fetch clubs if user is a store manager
      ...
      try {
        console.log('🏪 Fetching clubs for store:', storeId);

        // Store-scoped query - only fetch clubs belonging to this store
        ...
        console.log('✅ Fetched store clubs:', data?.length || 0);
        setClubs(data || []);
      } catch (error) {
        console.error('Error fetching store clubs:', error);
  ...
  // Helper function to determine if a club can be deleted by the current store manager
  const canDeleteClub = (club: BookClub): boolean => {
    if (!isStoreManager || !storeId) return false;

    // Store managers can only delete clubs in their own store
    return club.store_id === storeId;
  };
  ...
  // Show access denied if user is not a store manager
  if (!isStoreManager) {
    return (
      <div className="text-center p-8">
        <h2 className="text-xl font-semibold text-gray-900 mb-2">Access Denied</h2>
        <p className="text-gray-600">You must be a store manager to access club management.</p>
        <Button
          variant="outline"
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-serif text-bookconnect-brown mb-2">Club Management</h1>
        <p className="text-muted-foreground">
          Managing clubs for <span className="font-medium text-bookconnect-terracotta">{storeName}</span>
        </p>
      </div>
        ...
        {filteredClubs.length > 0 ? (
          ...
          <Card>
            <CardContent className="p-6 text-center">
              {clubs.length === 0 ? (
                <>
                  <p className="text-muted-foreground">No book clubs found for your store</p>
                  <Button
                    onClick={handleCreateClub}
...
export default StoreManagerClubsPage;
