...
-- Migration: 20250105_007_add_admin_utilities.sql
...
SELECT 
  ...
  -- System health
  (SELECT status FROM subscription_health_checks WHERE check_type = 'expired_subscriptions' ORDER BY checked_at DESC LIMIT 1) as last_health_status,
  (SELECT checked_at FROM subscription_health_checks WHERE check_type = 'expired_subscriptions' ORDER BY checked_at DESC LIMIT 1) as last_health_check,
...
BEGIN
  ...
  SELECT 
    id, email, username, membership_tier, account_tier, 
