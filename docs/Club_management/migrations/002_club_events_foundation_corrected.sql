-- =====================================================
-- Club Management Phase 3 - Events Integration Database Setup (CORRECTED)
-- Migration: 002_club_events_foundation_corrected.sql
-- Purpose: Create club meetings and event notification tables
-- Phase: 3 - Events Integration (Week 5)
--
-- CORRECTIONS:
-- - Uses 'users' table instead of 'profiles' table
...
BEGIN
  ...
  SELECT
    ...
    cm.created_at,
    cm.updated_at,
    COALESCE(u.username, u.displayname, u.email) as creator_username
...
-- This is required because PostgreSQL cannot change return types with CREATE OR REPLACE
...
BEGIN
  ...
  RAISE NOTICE 'Migration script: 002_club_events_foundation_corrected.sql';
  RAISE NOTICE 'CORRECTED: Uses public.users table instead of profiles table';
END $$;